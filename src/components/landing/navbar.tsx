...
import { FileText, Plus, LogOut, Menu, X } from "lucide-react";
    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3">
          ...
          {user ? (
            ...
            <button
              onClick={handleCriarContrato}
              className="hidden md:block rounded-xl px-5 py-2 text-sm font-semibold transition-all duration-200 hover:scale-[1.03] active:scale-[0.97]"
              style={{ background: "#d4ff00", color: "#050b18" }}
            >
              Criar contrato
            </button>
          ...
          <button
            ...
            style={{ color: "#ffffff", background: "rgba(255,255,255,0.07)" }}
      ...
      {mobileOpen && (
        <div
          ...
          style={{ background: "rgba(5,11,24,0.97)", paddingTop: "72px" }}
          onClick={(e) => { if (e.target === e.currentTarget) setMobileOpen(false); }}
        >
          <div className="flex flex-col px-5 pt-4 pb-8 gap-2">
            {user ? (
              <>
                {/* Info do usuário */}
                <div
                ...
                {NAV_LINKS.map((link) =>
                  link.highlight ? (
                    <button
                      key={link.href}
                      onClick={handleCriarContrato}
                      className="flex items-center gap-2 rounded-2xl px-4 py-3.5 text-base font-semibold transition-all"
                      style={{ background: "#d4ff00", color: "#050b18" }}
                    >
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-2xl px-4 py-3.5 text-base mt-2 transition-colors"
                  style={{
              ...
              <button
                onClick={handleCriarContrato}
                className="flex items-center justify-center rounded-2xl px-4 py-3.5 text-base font-semibold transition-all"
                style={{ background: "#d4ff00", color: "#050b18" }}
              >
                Criar contrato
              </button>
